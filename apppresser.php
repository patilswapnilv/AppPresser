...
class AppPresser {
	...
	function do_appp_script() {
		...
		if ( is_user_logged_in() || self::is_app() ) {
			...
			wp_enqueue_script( 'appp-core', self::$js_url . "appp$min.js", array( 'jquery' ), self::VERSION );
			wp_localize_script( 'appp-core', 'apppCore', self::$l10n );
			return;
		}
		if ( ! self::is_app() ) {
			wp_enqueue_script( 'appp-no-app', self::$js_url . 'no-app.js', array( 'jquery' ), self::VERSION );
			return;
		}
	...
	public static function get_apv() {
		...
		} elseif ( ( isset( $_GET['appp_bypass'] ) && $_GET['appp_bypass'] == 'true' ) || ( isset( $_COOKIE['AppPresser_Bypass'] ) && $_COOKIE['AppPresser_Bypass'] == 'true' ) ) {
			if ( isset( $_GET['appp_bypass'] ) ) {
				self::set_bypass_cookie();
			}
		} elseif ( isset( $_GET['appp'] ) && $_GET['appp'] == 3 || isset( $_COOKIE['AppPresser_Appp3'] ) && $_COOKIE['AppPresser_Appp3'] === 'true' ) {
			return 3;
		} elseif ( isset( $_GET['appp'] ) && $_GET['appp'] == 2 || isset( $_COOKIE['AppPresser_Appp2'] ) && $_COOKIE['AppPresser_Appp2'] === 'true' ) {
			return 2;
		} elseif ( ( isset( $_GET['appp'] ) && $_GET['appp'] == 1 ) || isset( $_COOKIE['AppPresser_Appp'] ) && $_COOKIE['AppPresser_Appp'] === 'true' ) {
			return 1;
		}
	...
	public static function set_deprecate_version( $deprecate_ver = null ) {
		...
		} elseif ( isset( $_GET['appp_deprecate_ver'] ) && is_numeric( $_GET['appp_deprecate_ver'] ) ) {
